...
class APIClient:
    ...
    def ask_grok(self, prompt, temp=0.3, max_tokens=1024, use_cache=True, max_retries=3, retry_delay=2):
        """
        Отправляет запрос к Gemini API и возвращает ответ с механизмом повторных попыток.
        ...
        while retries < max_retries:
            try:
                ...
                if retries == max_retries:
                    ...
                    self.logger.warning(f"Ошибка при запросе к API Gemini. Попытка {retries}/{max_retries}. Повторная попытка через {retry_delay} секунд...")
