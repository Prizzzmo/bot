...
class AdminPanel:
    ...
    def _load_admins(self):
        """Загружает список администраторов из файла или создает его"""
        try:
            if os.path.exists('admins.json'):
                with open('admins.json', 'r', encoding='utf-8') as f:
                    return json.load(f)
            else:
                # Создаем файл с администраторами по умолчанию, если он отсутствует
                default_admins = {
                    "admin_ids": [12345678],  # Замените на ID вашего Telegram аккаунта
                    "super_admin_ids": []
                }
            ...
            self.logger.error(f"Ошибка при загрузке файла администраторов: {e}")
    ...
    def save_admins(self):
        ...
        try:
            ...
            self.logger.error(f"Ошибка при сохранении файла администраторов: {e}")
            return False
