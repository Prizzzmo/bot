    ...
    def _split_content_into_chapters(self, content, chapters, topic):
        ...
        for i, chapter in enumerate(chapters, 1):
            if chapter in chapter_contents and chapter_contents[chapter]:
                ...
                # Добавляем заголовок с выделением
                chapter_message = f"{emoji} *Глава {i}: {chapter}*\n\n"
                
                # Разбиваем текст главы на абзацы для лучшей читаемости
                paragraphs = re.split(r'\n{2,}', chapter_text.strip())
                ...
                for paragraph in paragraphs:
                    ...
                    # Выделяем имена исторических личностей, которые идут после слов "царь", "император", и др.
                    clean_paragraph = re.sub(r'(царь|император|князь|королева|премьер-министр) ([А-Я][а-я]+ [А-Я][а-я]+)', r'\1 *\2*', clean_paragraph)
                    
                    formatted_paragraphs.append(clean_paragraph)
                
                # Соединяем абзацы с пустой строкой между ними
                ...
                emoji = chapter_emoji.get(chapter, "•")
                empty_chapter_message = f"{emoji} *Глава {i}: {chapter}*\n\n"
