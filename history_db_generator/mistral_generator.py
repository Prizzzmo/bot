...
MISTRAL_API_KEY = "RfyJs8n0Cr7LCLBJ72jI8KcBYHkV96OL"

# Константы для работы скрипта
DB_FILE = "history_db_generator/russian_history_database_mistral.json"
TEMP_FOLDER = "history_db_generator/temp_mistral"
BACKUPS_FOLDER = "history_db_generator/backups_mistral"

# Семафор для ограничения частоты запросов (1 запрос в секунду)
...
def ensure_directories():
    ...
    print(f"Директории подготовлены: history_db_generator, {TEMP_FOLDER}, {BACKUPS_FOLDER}")

def hash_string(text):
    """Создает хеш строки для использования в идентификаторах."""
    return hashlib.md5(text.encode()).hexdigest()[:8]

def call_mistral_api(prompt, temperature=0.3, max_tokens=1024, retry_count=5, retry_delay=10):
    """
    Отправляет запрос к API Mistral с механизмом повторных попыток и ограничением частоты.
    ...
    for attempt in range(retry_count):
        try:
            # Используем семафор для ограничения частоты запросов
            with request_semaphore:
                print(f"Отправка запроса к Mistral API (попытка {attempt+1}/{retry_count})...")
                ...
                # Обязательная задержка в 1 секунду после запроса
                time.sleep(1)
            ...
            if attempt < retry_count - 1:
                ...
                if "429" in error_str or "too many" in error_str or "rate" in error_str:
                    # Для ошибок превышения лимита запросов делаем более длительную задержку
...
def generate_historical_topics():
    ...
    # Создаем запрос для получения всеобъемлющего списка тем
    prompt = """
    ...
    4. Правление каждого из русских царей и императоров
    5. Смутное время
    6. Петровские реформы
    7. Дворцовые перевороты XVIII века
    8. Российская империя в XIX веке
    ...
    Представь результат в виде нумерованного списка.
    """
    
    # Делаем несколько запросов для получения максимального количества тем
    all_topics = []
    batch_count = 3  # Количество разных запросов для максимального охвата
    ...
    for i in range(batch_count):
        ...
        response = call_mistral_api(batch_prompt, temperature=0.7, max_tokens=2048)
    ...
    # Если мы получили слишком мало тем, добавим некоторые ключевые
    if len(all_topics) < 50:
        # Список известных исторических тем
        known_topics = [
            ...
            "Распад СССР", "Экономические реформы 1990-х", "Присоединение Крыма"
        ]
...
def get_events_for_topic(topic):
    ...
    Приведи не менее 10-15 событий, если это возможно для данной темы. Будь максимально точным в датах и местах.
    """
    
    # Выполняем запрос к API
    response = call_mistral_api(events_prompt, temperature=0.3, max_tokens=2048)
    ...
    if len(response) < 500:
        ...
        Постарайся включить не менее 10-15 событий, охватывающих весь хронологический период темы.
        """
        response = call_mistral_api(alternative_prompt, temperature=0.4, max_tokens=2048)
    ...
    if not events:
        ...
        events = extract_events_from_text(response)
        ...
        for event in events:
            if "location" not in event:
                ...
                location = call_mistral_api(location_prompt, temperature=0.1, max_tokens=100)
...
def generate_database():
    ...
    """
    print("Начало генерации базы данных исторических событий России с помощью Mistral AI...")
    ...
    # Ограничиваем количество обрабатываемых тем для демонстрации
    max_topics = min(len(topics), 50)  # Ограничиваем 50 темами для демонстрации
    
    for i, topic in enumerate(topics[start_index:max_topics], start=start_index):
        try:
            print(f"Обработка темы {i+1}/{max_topics}: {topic}")
            ...
            if i % save_interval == 0 or i == max_topics - 1:
    ...
    backup_file = f"{BACKUPS_FOLDER}/russian_history_database_backup_{int(time.time())}.json"
    save_json(database, backup_file)
...
if __name__ == "__main__":
    print("=== Генератор базы данных исторических событий России (Mistral AI) ===")
    print(f"Используется API ключ Mistral: {MISTRAL_API_KEY[:5]}...{MISTRAL_API_KEY[-5:]}")
    ...
    try:
        ...
        test_response = call_mistral_api("Тестовый запрос: ответь коротко 'API работает'")
        if "работает" in test_response.lower():
            print("API Mistral работает корректно.")
        else:
